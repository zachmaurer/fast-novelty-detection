...
def loadTfRecordDataset(image_paths, label_fn = False):
  ...
  return loadDataset(image_paths, tf.contrib.data.TFRecordDataset, openTfRecordImage, label_fn)

def loadJpegDataset(image_paths, label_fn = None):
  """ Loads image dataset from a list of directories containing image files.  """
  return loadDataset(image_paths, tf.contrib.data.Dataset.from_tensor_slices, openImageTf, label_fn)

def loadDataset(image_paths, tf_dataset, file_opener, label_fn):
  """
  ...
  for p in image_paths:
    filenames += [path.join(p, x)  for x in os.listdir(p) if not x.startswith('.')]
  ...
  if label_fn is not None and not isinstance(label_fn, bool):
    ...
    dataset = dataset.map(lambda x, y: (file_opener(x), y))
  else:
    dataset = tf_dataset((input_imgs))
    dataset = dataset.map(lambda x: file_opener(x, return_labels = label_fn))
  return dataset
